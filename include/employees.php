...
class Employee {
	...
	public function create() {
		...
		try {
			...
			// Remove INCID as it's auto-increment
			unset($attributes['INCID']);
			...
			error_log("SQL Query for tblemployees: " . $sql); // Debug log
			
			if($mydb->setQuery($sql) && $mydb->executeQuery()) {
				$this->INCID = $mydb->insert_id();
				error_log("Employee created with INCID: " . $this->INCID); // Debug log
				return true;
			} else {
				error_log("Database error in tblemployees: " . $mydb->getError()); // Debug log
				return false;
			}
		} catch(Exception $e) {
			error_log("Exception in create() for tblemployees: " . $e->getMessage());
			return false;
		}
	}

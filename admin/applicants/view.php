...
global $mydb;
	...
	// Check if applicant has any job applications
	$sql = "SELECT * FROM `tbljobregistration` WHERE APPLICANTID = '{$red_id}'";
	$mydb->setQuery($sql);
	$jobreg = $mydb->loadSingleResult();

	if($jobreg) {
		$jobregistration = New JobRegistration();
		$jobreg = $jobregistration->single_jobregistration($jobreg->REGISTRATIONID);
...
<div class="col-sm-<?php echo $jobreg ? '6' : '12'; ?> content-body" >
